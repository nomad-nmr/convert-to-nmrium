...
function migrateToVersion2(data) {
    ...
    for (const spectrum of newData.spectra) {
        if (spectrum.peaks) {
            spectrum.peaks = (0, changeKeys_1.changeKeys)(spectrum.peaks, changedKeys.peaks);
        }
        else if (spectrum.ranges) {
            spectrum.ranges = (0, changeKeys_1.changeKeys)(spectrum.ranges, changedKeys.ranges);
        }
...
exports.default = migrateToVersion2;
function mapZones(zones) {
    return zones.map((zone) => {
        if (zone.signals) {
            zone.signals = zone.signals.map((signal) => {
                if (signal.x && signal.y) {
                    const { x: { fromTo: fromToX, ...resX }, y: { fromTo: fromToY, ...resY }, } = signal;
                    signal = {
                        ...signal,
                        x: { ...resX, ...fromToX },
                        y: { ...resY, ...fromToY },
                    ...
                    const { fromTo, deltaX, resolutionX, nucleusX, deltaY, resolutionY, nucleusY, shiftX, shiftY, ...resSignal } = signal;
                    signal = {
                        ...resSignal,
                        x: {
                            from: fromTo[0].from,
