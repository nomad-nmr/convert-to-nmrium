...
exports.defaultContourOptions = void 0;
const ml_conrec_1 = require("ml-conrec");
exports.defaultContourOptions = {
    positive: {
        contourLevels: [0, 21],
        numberOfLayers: 10,
    },
...
class Processing2D {
    constructor(minMax, options = exports.defaultContourOptions, defaultLevel = {
        positive: 10,
        negative: 10,
    }) {
        ...
        this.conrec = new ml_conrec_1.Conrec(minMax.z, { xs, ys, swapAxes: false });
    ...
    wheel(value) {
        ...
        if ((this.currentLevelPositive > positiveBoundary[0] && sign === -1) ||
            (this.currentLevelPositive < positiveBoundary[1] && sign === 1)) {
            this.currentLevelPositive += sign;
        }
        if ((this.currentLevelNegative > negativeBoundary[0] && sign === -1) ||
    ...
    shiftWheel(value) {
        ...
        if ((this.currentLevelPositive === min && sign === -1) ||
    ...
    drawContours() {
        ...
        const { positive: { numberOfLayers: numberOfPositiveLayer }, negative: { numberOfLayers: numberOfNegativeLayer }, } = this.options;
        return {
    ...
    getContours(zoomLevel, options = {}) {
        ...
        let _range = getRange(this.median * 1 * Math.pow(2, zoomLevel), max, nbLevels, 2);
        if (negative) {
